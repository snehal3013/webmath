...
public class TC_webmath_051 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Sheet s = wb.getSheet("TC_webmath_051");
			...
			WebElement f1 = driver.findElement(By.name(s.getCell(1, 13).getContents()));

			driver.switchTo().frame(f1);
			Thread.sleep(2000);

			if (driver.getPageSource().contains(s.getCell(1, 14).getContents())) {
				System.out.println(s.getCell(1, 15).getContents());
			} else {
				System.out.println(s.getCell(1, 16).getContents());
			}
		} catch (Exception e) {
