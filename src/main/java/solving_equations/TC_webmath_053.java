...
public class TC_webmath_053 {

	public static void main(String[] args) {
		WebDriver driver = WebDriverManager.chromedriver().create();
		try {
			FileInputStream f = new FileInputStream("D:\\ExcelR Webmath Project\\Solving_Equations.xls");
			Workbook wb = Workbook.getWorkbook(f);

			Sheet s = wb.getSheet("TC_webmath_053");
			...
			WebElement f1 = driver.findElement(By.name(s.getCell(1, 19).getContents()));

			driver.switchTo().frame(f1);
			Thread.sleep(2000);

			if (driver.getPageSource().contains(s.getCell(1, 20).getContents())) {
				System.out.println(s.getCell(1, 21).getContents());
			} else {
				System.out.println(s.getCell(1, 22).getContents());
			}
		} catch (Exception e) {
