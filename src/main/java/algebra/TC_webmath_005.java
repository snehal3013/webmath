...
public class TC_webmath_005 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Sheet s = wb.getSheet("TC_webmath_005");

			driver.get(s.getCell(1, 0).getContents());
			Thread.sleep(2000);
			driver.manage().window().maximize();

			boolean searchbar = driver.findElement(By.xpath(s.getCell(1, 1).getContents())).isEnabled();
			if (searchbar == true) {
			...
			boolean go = driver.findElement(By.xpath(s.getCell(1, 4).getContents())).isEnabled();
			if (go == true) {
